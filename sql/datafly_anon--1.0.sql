...
LANGUAGE plpgsql STRICT
	...
	DECLARE 
		...
		target_tbl_name := json_config ->>'targetTableName';
 		quasi_identifiers := json_config->'qi_attributes';
		for counter in 0 .. json_array_length(quasi_identifiers)-1 loop
			...
			for counter in 0 ..json_array_length(quasi_identifiers_generalization)-1 loop
				perform add_level_generalization(sch_name,quasi_identifiers_info->>'attrName'::VARCHAR,tbl_name, quasi_identifiers_generalization->counter->>'generalization_rule'::VARCHAR, CAST(quasi_identifiers_generalization->counter->>'level' AS INTEGER), quasi_identifiers_generalization->counter->>'generalizationFunction'::VARCHAR,target_sch_name,target_tbl_name);
				raise notice 'values: %, %, %, %', quasi_identifiers_generalization->counter->>'level', quasi_identifiers_generalization->counter->>'generalization_rule',quasi_identifiers_generalization->counter->>'generalizationFunction',sch_name;				
				end loop;
		end loop;
		END;
	$$;
